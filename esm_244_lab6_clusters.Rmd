...
```{r}
summary(penguins) # looking at the unit scales for the variables
# k-means cluster analysis won't work because the numbers aren't equivalent scales
...
penguins_scale <- penguins_complete %>% 
  select(ends_with("mm"), body_mass_g) %>% # selects all variables that ends with mm
...
number_est <- NbClust(penguins_scale, min.nc = 2, max.nc = 10, method = "kmeans") # giving a range of estimates for number of clusters
...
### knee method
fviz_nbclust(penguins_scale, FUNcluster = kmeans, method = 'wss', k.max = 10) # wss = within sum of squares
```
...
```{r}
# could also set a seed to standardize random clusters

penguins_km <- kmeans(penguins_scale, centers = 3, nstart = 25) # kmeans specifying 3 groups to start
...
# Do the same, plotting bill dimensions and mapping species & cluster number to the point shape and color aesthetics:
...
## Part 2. Cluster analysis: hierarchical (agglomerative)
...
# also try method = single, average, ward.D
...
This is UNSUPERVISED machine learning while regression is SUPERVISED. 
...
wb_ghg_20 <- wb_env %>% 
  arrange(-ghg) %>% # arrange by GHG emissions in descending order
  head(20) # only retain the top 20 observations
...
# need to re-add the country names because can't scale characters 
...
euc_distance <- dist(wb_scaled, method = "euclidean") ### add diag and upper, diag = TRUE, upper = TRUE
